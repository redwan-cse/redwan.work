...
import { PaymentForm, PrintInvoiceButton } from '@/components/portal/payment-form';
...
export default async function PortalInvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) redirect(`/login?next=/portal/invoices/${id}`);
  const result = await getInvoiceDetail(id, { userId: session.userId, role: 'client' });
  if (!result.ok || !('invoice' in result)) notFound();
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3"><Badge variant="outline">{invoice.status}</Badge><PrintInvoiceButton /></div>
      </header>
      <main className="space-y-6">
        <section className="grid gap-4 sm:grid-cols-3"><div><p className="text-xs uppercase text-muted-foreground">Issued</p><p>{date(invoice.issued_at)}</p></div><div><p className="text-xs uppercase text-muted-foreground">Due</p><p>{date(invoice.due_at)}</p></div><div><p className="text-xs uppercase text-muted-foreground">Currency</p><p>{invoice.currency}</p></div></section>
        <section className="space-y-3"><h2 className="text-lg font-semibold">Line items</h2><div className="overflow-x-auto rounded-lg border"><table className="w-full text-sm"><thead className="bg-muted/50 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-2">Description</th><th className="px-4 py-2 text-right">Qty</th><th className="px-4 py-2 text-right">Unit price</th><th className="px-4 py-2 text-right">Amount</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t"><td className="px-4 py-2">{item.description}</td><td className="px-4 py-2 text-right">{item.qty}</td><td className="px-4 py-2 text-right">{money(item.unit_price_cents, invoice.currency)}</td><td className="px-4 py-2 text-right">{money(Math.round(item.qty * item.unit_price_cents), invoice.currency)}</td></tr>)}</tbody></table></div></section>
        ...
        <section><h2 className="mb-3 text-lg font-semibold">Payment history</h2>{payments.length === 0 ? <p className="text-sm text-muted-foreground">No payments submitted.</p> : <div className="overflow-x-auto rounded-lg border"><table className="w-full text-sm"><thead className="bg-muted/50 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-2">Date</th><th className="px-4 py-2">Method</th><th className="px-4 py-2">Reference</th><th className="px-4 py-2 text-right">Amount</th><th className="px-4 py-2">Status</th></tr></thead><tbody>{payments.map((payment) => <tr key={payment.id} className="border-t"><td className="px-4 py-2">{date(payment.created_at)}</td><td className="px-4 py-2">{payment.method}</td><td className="px-4 py-2 font-mono text-xs">{payment.reference}</td><td className="px-4 py-2 text-right">{money(payment.amount_cents, invoice.currency)}</td><td className="px-4 py-2">{payment.status}</td></tr>)}</tbody></table></div>}</section>
        {invoice.status === 'sent' && invoice.outstanding_cents > 0 && <PaymentForm invoiceId={invoice.id} outstandingCents={invoice.outstanding_cents} currency={invoice.currency} />}
      </main>
    </div>
  );
}
