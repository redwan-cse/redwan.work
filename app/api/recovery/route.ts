...
type ImportRow={id:string;actor:string;upload_key:string;sealed_key:string;sha256:string|null;kind:'individual'|'project'|null;snapshot:unknown;result:unknown;created_at:string};
function fileRows(row:ImportRow):FileRow[]{const s=row.snapshot as {files?:FileRow[]};const files=row.kind==='project'?s.files:[row.snapshot as FileRow];if(!Array.isArray(files)||files.length>2000)throw new Error();for(const f of files)if(!f||!UUID.test(f.id)||typeof f.filename!=='string'||typeof f.mime!=='string'||!Number.isSafeInteger(Number(f.size_bytes))||Number(f.size_bytes)<1)throw new Error();return files;}
function newId(operation:string,source:string){const h=createHash('sha256').update(operation+':'+source).digest('hex');return `${h.slice(0,8)}-${h.slice(8,12)}-4${h.slice(13,16)}-8${h.slice(17,20)}-${h.slice(20,32)}`;}
async function readImport(id:string,actor:string):Promise<ImportRow>{const r=await getSupabaseAdmin().from('recovery_imports').select('id,actor,upload_key,sealed_key,sha256,kind,snapshot,result,created_at').eq('id',id).eq('actor',actor).maybeSingle();if(r.error||!r.data)throw new Error();return r.data as ImportRow;}
...
export async function POST(request:NextRequest){
 try{
  ...
  const raw=await request.text();if(Buffer.byteLength(raw)>8192)return failure();const input:unknown=JSON.parse(raw);
  ...
  if(data.action==='preview'){
   ...
   // Seal a separate key with no browser-issued PUT URL. Later upload overwrites
   // cannot alter the verified restore input. Only registered hashes qualify.
  ...
  if(data.action==='restore'){
   if(row.result)return reply({result:row.result});const zip=await readRecoveryBytes(row.sealed_key);const entries=verifyEntries(row,zip);const files=fileRows(row);
   ...
   if(!UUID.test(owner)||(!project&&!ticket))return failure();const mapping:{project_id?:string;files:Array<{source_id:string;id:string;key:string}>}={...(row.kind==='project'?{project_id:project!}:{}),files:[]};
   for(const f of files){const ext=f.r2_key.match(/\.(pdf|docx|doc|xlsx|png|jpg|zip)$/)?.[1];if(!ext)return failure();const id=newId(row.id,f.id),key=`private/${owner}/${project?'project_'+project:'ticket_'+ticket}/${id}.${ext}`;const bytes=entries.get(`files/${f.id}`)!;await writeRestoredObject(key,bytes,f.mime);if(hash(await readRecoveryBytes(key,bytes.length))!==hash(bytes))return failure();mapping.files.push({source_id:f.id,id,key});}
   // SQL rechecks authority, parents and all mapping fields; duplicate confirms
   // serialize on the durable import row and return the first committed result.
