...
export default async function AdminTicketThreadPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{cursor?:string|string[]}>}){
 const {id}=await params;const {cursor}=await searchParams;const base=`/admin/tickets/${encodeURIComponent(id)}`;
 const session=await getCurrentSession();if(!session)redirect('/login');if(session.role!=='admin')redirect('/portal');
 const result=await getTicketThread(id,cursor);
 ...
 return <div className="space-y-6"><Link href="/admin/tickets" className="text-sm text-muted-foreground hover:underline">All tickets</Link>
 <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono text-xs text-muted-foreground">#TKT-{ticket.number}</p><h1 className="text-xl font-semibold">{ticket.subject}</h1><p className="mt-1 text-sm text-muted-foreground">{ticket.client_name??'Client'} · {ticket.client_email}</p></div><div className="flex items-center gap-3"><Badge className={COLORS[ticket.status]}>{LABELS[ticket.status]}</Badge><StatusControl ticketId={ticket.id} status={ticket.status}/></div></header>
 <ThreadNavigation base={base} pageInfo={pageInfo}/>
 <section id="conversation" tabIndex={-1} aria-label="Conversation" className="space-y-3">{messages.length===0&&<p>No messages on this page. Use Latest messages to refresh the conversation.</p>}{messages.map(message=><article key={message.id} data-message-id={message.id} className="rounded-lg border p-4"><div className="mb-2 flex flex-wrap items-baseline justify-between gap-x-4 text-xs"><span className="font-medium">{message.author_name??(message.author_role==='admin'?'Admin':'Client')}</span><time dateTime={message.created_at} className="text-muted-foreground">{new Date(message.created_at).toISOString().slice(0,16).replace('T',' ')} UTC</time></div><p className="whitespace-pre-wrap break-words text-sm">{message.body}</p></article>)}</section>
