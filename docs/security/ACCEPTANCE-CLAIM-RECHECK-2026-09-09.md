...
- **Audit Follow-up Remediation** (Branch `fix/audit-followup-remediation`):
  ...
  2. **Recovery / Invite Session Proof Binding**: In `lib/auth/actions.ts`, password update retry after single-use token consumption binds a cryptographic session proof cookie (`recovery_proof` / `invite_proof`) with 300s TTL. Retries verify active session, proof cookie, and token hash match before `updateUser`. Rejects unrelated sessions, expired sessions, wrong users, and wrong tokens fail-closed. Verified by stateful tests in `tests/reliability/recovery-controls.test.mjs`.
