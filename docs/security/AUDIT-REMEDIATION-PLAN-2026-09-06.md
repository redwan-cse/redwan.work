...
2. **Recovery & Invite Session-Bound Retry Authority** (`lib/auth/actions.ts`, `supabase/migrations/0036_auth_retry_claims.sql`):
   ...
   - Verified by unit and concurrency tests in `tests/reliability/recovery-controls.test.mjs` and real disposable PostgreSQL tests across separate workers in `tests/reliability/auth-retry-claims-db.py`.
...
| **F02** | [#28](https://github.com/redwan-cse/redwan.work/issues/28) | Session lifecycle and recoverable transitions: recovery and invite acceptance enforce server-authenticated, expiring, session-bound retry authority with durable single-use claims and fixed expiry, rejecting forged cookies, cross-device session mismatch, and cross-instance/concurrent replay attempts without resetting windows or reopening after ambiguous provider failures; canonical relative return paths prevent open redirects. | `lib/auth/actions.ts` (`setNewPasswordFromRecoveryAction`, `acceptInviteAction`, `signInWithPasswordAction`, `safeRelativePath`), `supabase/migrations/0036_auth_retry_claims.sql` | `tests/reliability/recovery-controls.test.mjs`, `tests/reliability/auth-retry-claims-db.py`, `tests/reliability/login-return.test.mjs`, `tests/reliability/account-lifecycle.acceptance.mjs` | Rejects password < 12 chars before token consumption; verifies HMAC-signed expiring session authority bound to session ID; atomically claims single-use nonce via `claim_auth_retry_nonce` with zero fail-open memory fallback; permanently consumes authority without reopening on ambiguous provider failures; rejects forged cookies, tampered payloads, session hijacking, cross-instance and concurrent replay fail-closed; enforces canonical relative return paths (`A01`). |
