...
2. **Recovery & Invite Session-Bound Retry Authority** (`lib/auth/actions.ts`, `supabase/migrations/0036_auth_retry_claims.sql`):
   - Server-authenticated, expiring, session-bound retry authority cookie (`recovery_retry_authority` / `invite_retry_authority`) HMAC-signed with `SUPABASE_SECRET_KEY` (legacy secret-key fallback removed), binding user ID (`sub`), token hash (`tok`), session ID (`sid`), expiration (`exp`), and atomic single-use nonce (`nonce`).
   - On retry after single-use OTP consumption, verifies signature, expiration, active session user ID, session ID, and token hash before invoking `updateUser`. Rejects forged cookies, tampered signatures/payloads, expired sessions, wrong users, and wrong session IDs fail-closed with `INVALID_LINK`.
   - Durable single-use claims with fixed expiry (`consumeAtomicRetryClaim`) insert into `public.auth_retry_claims` via `claim_auth_retry_nonce` RPC. Eliminates fail-open memory fallbacks and resetting rate-limit windows. Once claimed, authority is permanently consumed and never reopened after ambiguous provider failures.
   - **Direct Table Privilege Revocation & Runtime Access Restriction**: Direct table privileges on `public.auth_retry_claims` are completely revoked from `public, anon, authenticated, service_role`. Direct mutations (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`) fail closed with SQLSTATE `42501` (`insufficient_privilege`). `EXECUTE` on `claim_auth_retry_nonce` and `cleanup_expired_auth_retry_claims` is revoked from `public, anon, authenticated` and granted strictly to `service_role`. Runtime check raises exception if called with `anon` or `authenticated` JWT role.
   - **Expiry-Based Cleanup**: Table index `auth_retry_claims_expires_at_idx` supports efficient range deletion. A 7-day retention grace period (`expires_at < now() - interval '7 days'`) ensures spent claims permanently resist replay across delays or clock skew, before being purged opportunistically by `claim_auth_retry_nonce` and by scheduled sweeps via `cleanup_expired_auth_retry_claims()`.
   - **Fresh-Link Recovery Path**: Replaces futile "Try again" copy after a spent claim with a clear recovery path: `'Could not update your password. This reset link is no longer valid. Please request a new password reset link.'` with structured `{linkHref: '/login', linkText: 'Request a new password reset link'}`.
   - Verified by unit and concurrency tests in `tests/reliability/recovery-controls.test.mjs` and real disposable PostgreSQL tests verifying actual role calls, SQLSTATE 42501 denials on service-role and untrusted roles, clean `SET` output parsing, and separate-worker concurrency in `tests/reliability/auth-retry-claims-db.py`.
...
|---|---|---|---|---|---|
| **F01** | [#29](https://github.com/redwan-cse/redwan.work/issues/29) | Authorization lifecycle verification: active vs inactive accounts separated, cross-tenant/cross-client boundaries strictly enforced, and fail-closed RLS policies across DB tables. | `proxy.ts`, `lib/crm/workflow-access.ts`, `lib/crm/client-actions.ts`, `lib/crm/admin-actions.ts`, `lib/crm/tickets.ts`, `lib/crm/invoices.ts`, DB migrations `0001`–`0036` | `tests/reliability/caller-inventory.test.mjs`, `tests/reliability/admin-boundaries.test.mjs`, `tests/reliability/invoice-server-boundary.test.mjs`, `tests/reliability/workflows.test.mjs` | Unauthenticated or unauthorized callers receive deterministic rejection; client users cannot access admin endpoints; inactive accounts fail closed. |
| **F02** | [#28](https://github.com/redwan-cse/redwan.work/issues/28) | Session lifecycle and recoverable transitions: recovery and invite acceptance enforce server-authenticated, expiring, session-bound retry authority with durable single-use claims and fixed expiry; direct runtime table privileges revoked from all roles (including service_role); service_role DELETE/UPDATE/TRUNCATE denied (SQLSTATE 42501); expiry-based cleanup accurately enforced with 7-day retention; ambiguous provider failures never reopen spent claims, providing a clear fresh-link recovery path instead of futile retries; canonical relative return paths prevent open redirects. | `lib/auth/actions.ts` (`setNewPasswordFromRecoveryAction`, `acceptInviteAction`, `signInWithPasswordAction`, `safeRelativePath`), `supabase/migrations/0036_auth_retry_claims.sql`, `components/auth/auth-form.tsx` | `tests/reliability/recovery-controls.test.mjs`, `tests/reliability/auth-retry-claims-db.py`, `tests/reliability/login-return.test.mjs`, `tests/reliability/account-lifecycle.acceptance.mjs` | Rejects password < 12 chars before token consumption; verifies HMAC-signed expiring session authority bound to session ID; atomically claims single-use nonce via `claim_auth_retry_nonce` with zero fail-open memory fallback; revokes public and service_role direct table privileges; verifies service-role DELETE/UPDATE/TRUNCATE denial with SQLSTATE 42501; permanently consumes authority without reopening on ambiguous provider failures; replaces 'Try again' with fresh-link recovery path; purges claims > 7 days past expiry via `cleanup_expired_auth_retry_claims`; verifies separate-worker concurrency; enforces canonical relative return paths (`A01`). |
...
| **F10** | [#36](https://github.com/redwan-cse/redwan.work/issues/36) | Partial object-deletion reconciliation: storage object deletions handle partial failure and provider errors without losing track of un-deleted files; database state remains consistent with storage. | `lib/crm/files.ts`, `lib/r2.ts`, `lib/crm/file-deletion.ts` | `tests/reliability/file-deletion.test.mjs`, `tests/reliability/file-deletion-order.test.mjs` | Failed object deletions retain file records for subsequent cleanup retries; successful deletions prune metadata atomically. |
...
| **F18** | [#46](https://github.com/redwan-cse/redwan.work/issues/46) | Structured diagnostic redaction: structured logging and error diagnostics redact sensitive client tokens, passwords, session hashes, email contents, and financial identifiers before logging or telemetry sinks. | `lib/crm/email-log.ts`, `lib/contact/lead-store.ts`, `app/api/uploads/presign/route.ts`, `app/api/revalidate/route.ts`, `tests/r01-redaction.mjs` | `tests/reliability/diagnostic-redaction.test.mjs`, `tests/reliability/lead-diagnostics.test.mjs`, `tests/r01-redaction.mjs` | PII, tokens, and raw credentials in error objects are scrubbed with `[REDACTED]` markers; log output contains only safe diagnostic categories. |
| **F19** | [#47](https://github.com/redwan-cse/redwan.work/issues/47) | Attachment sharing/removal semantics: ticket attachment sharing, removal, and download URLs adhere strictly to access control rules; client removal does not delete shared files permanently without confirmation. | `lib/crm/attachments.ts`, `lib/crm/files.ts`, `app/api/files/[id]/download/route.ts` | `tests/reliability/ticket-attachments.test.mjs`, `tests/reliability/download-contract.test.mjs`, `tests/reliability/file-authority.test.mjs` | Presigned download URLs are scoped to authorized ticket holders; removal marks attachment records and respects ownership permissions. |
