...
| **F02** | [#28](https://github.com/redwan-cse/redwan.work/issues/28) | Session lifecycle and recoverable transitions: recovery and invite acceptance enforce server-authenticated, expiring, session-bound retry authority, rejecting forged cookies and replay attempts; canonical relative return paths prevent open redirects. | `lib/auth/actions.ts` (`setNewPasswordFromRecoveryAction`, `acceptInviteAction`, `loginAction`, `getValidatedReturnPath`) | `tests/reliability/recovery-controls.test.mjs`, `tests/reliability/login-return.test.mjs`, `tests/reliability/account-lifecycle.acceptance.mjs` | Rejects password < 12 chars before token consumption; verifies HMAC-signed expiring session authority; rejects forged cookies, tampered payloads, and replay attempts fail-closed; enforces canonical relative return paths (`A01`). |
