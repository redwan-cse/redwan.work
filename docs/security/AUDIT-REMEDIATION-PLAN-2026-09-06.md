...
2. **Recovery & Invite Session-Bound Retry Authority** (`lib/auth/actions.ts`, `supabase/migrations/0036_auth_retry_claims.sql`):
   ...
   - **Public Execution Revocation & Runtime Access Restriction**: `EXECUTE` on `claim_auth_retry_nonce` and `cleanup_expired_auth_retry_claims` is explicitly revoked from `public, anon, authenticated` and granted strictly to `service_role`. Runtime check raises exception if called with `anon` or `authenticated` JWT role.
   ...
   - Verified by unit and concurrency tests in `tests/reliability/recovery-controls.test.mjs` and real disposable PostgreSQL tests verifying actual role calls and separate-worker concurrency in `tests/reliability/auth-retry-claims-db.py`.
