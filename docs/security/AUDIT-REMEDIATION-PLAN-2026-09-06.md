...
2. **Recovery & Invite Session-Bound Retry Authority** (`lib/auth/actions.ts`):
   ...
   - Shared atomic retry consumption (`consumeAtomicRetryNonce`) claims the nonce across serverless instances via `consume_rate_limit` (`auth-retry`, `p_max_count: 1`), defending against concurrent and cross-instance replay. Defined failure recovery (`releaseAtomicRetryNonce`) releases the reservation and refreshes authority if password update fails transiently, and deletes authority on success.
   - Verified by unit and concurrency tests in `tests/reliability/recovery-controls.test.mjs` covering tampered signatures, forged secrets, expired authority, session-ID hijacking, wrong users, wrong tokens, concurrent replay, cross-instance replay, and failure recovery.
