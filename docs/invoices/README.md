...
|---|---|---|---|---|
| 1 | Schema | Apply migrations through `0010`; inspect enums, policies, and helper privileges | Invoice/payment schema exists; financial helpers and RPCs are service-role-only | **Pass:** Task 1 reports migrations `0007`–`0008` applied; required enums/policies and helper privileges verified |
...
| 3 | Totals | Calculate line totals and invoice aggregates | Rounded line amounts; submitted includes submitted + confirmed; outstanding clamps at zero | **Pass:** Task 2 contract validation; Task 1 live fixture probe covered calculated total behavior |
...
| 10 | Atomic draft creation | Create draft with invalid project or item payload | No partial invoice or item rows | **Pass:** Task 3 reports migration `0010` applied and privilege check passed; authenticated fixture probe unavailable |
...
| 14 | Print | Print admin/client detail page | Invoice content remains; controls and portal chrome are hidden | **Pass:** Task 3 static print check and Task 4 print-style implementation check; print-media browser probe unavailable |
