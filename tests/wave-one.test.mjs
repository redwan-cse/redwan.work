...
for(const [i,path] of ['//outside.invalid','/\t/outside.invalid','/\n/outside.invalid','/\\outside.invalid','/portal','/portal\u001f','/portal\u007f','https://outside.invalid'].entries())test('A01 actual action rejects hostile destination '+i,async()=>{assert.equal(await destination(path),'/portal');});
test('A01 absent destination retains role fallback',async()=>{assert.equal(await destination(null),'/portal');});
for(const [i,entries,expected] of [
 [0,[],false],[1,[['ndaConfidentiality','']],false],[2,[['ndaConfidentiality','false']],false],[3,[['ndaConfidentiality','true']],true],[4,[['ndaConfidentiality','Yes - NDA or strict confidentiality required']],true]
])test('I01 accepted NDA vocabulary '+i,()=>{const r=parseLeadPayload(form(entries),meta);assert.equal(r.ok,true);assert.equal(r.lead.nda_required,expected);});
