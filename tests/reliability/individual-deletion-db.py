...
C='supabase_db_redwan-reliability-ci';a,u,foreign,p,t=[str(uuid.uuid4()) for _ in range(5)];phase='fixtures';ids=[];snapshots={}
def sql(s,ok=True):
...
def prepare(fid,actor,role):
 expected=snapshots[fid].replace("'","''");backup=f'archive/project_{fid}/individual_{fid}.zip'
 return f"select public.prepare_backed_up_file_deletion('{fid}','{actor}','{role}','{expected}'::jsonb,'{backup}','{'a'*64}',1000);"
def file(kind='deliverable',age='now()'):
 ...
 snapshots[f]=sql(f"select to_jsonb(f) from public.files f where id='{f}';")
 return f,key
try:
 ...
 sql(prepare(f,a,'admin'),False)
 assert sql(f"select count(*) from public.files where id='{f}';")=='1';assert sql(f"select count(*) from public.storage_deletions where file_id='{f}';")=='0';assert sql(f"select count(*) from public.file_recovery where file_id='{f}';")=='0'
 ...
 with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:results=list(pool.map(lambda _:sql(prepare(f,a,'admin')),range(4)))
 ...
 own,_=file('attachment');sql(prepare(own,foreign,'client'),False);sql(prepare(own,u,'client'))
 old,_=file('attachment',"now()-interval '25 hours'");sql(prepare(old,u,'client'),False)
 phase='archived parent and revoked role'
 archived,_=file();sql(f"update public.projects set archived_at=now() where id='{p}';");sql(prepare(archived,a,'admin'),False)
 sql(f"update public.profiles set is_active=false where id='{a}';");sql(prepare(f,a,'admin'),False)
 phase='privileges'
 for role in ['anon','authenticated','service_role']:assert sql(f"select has_function_privilege('{role}','public.prepare_file_deletion(uuid,uuid,text)','execute');")=='f'
 print('Passed: backed-up file deletion rollback, concurrent replay, tombstones, foreign/expired/archived/revoked denial and privileges. SQL proof fixtures are synthetic, not R2 verification.')
 ...
 sql(f"delete from public.storage_deletions where requested_by in('{a}','{u}','{foreign}');delete from public.file_recovery where requested_by in('{a}','{u}','{foreign}');delete from public.tickets where id='{t}';delete from public.projects where id='{p}';delete from public.email_outbox where recipient_id in('{a}','{u}','{foreign}');delete from auth.users where id in('{a}','{u}','{foreign}');")
