...
for name,command in stages:
    ...
    if result.returncode:
        # Only source locations and compiler codes leave private child output.
        matches=re.findall(r'([\w/().\[\]-]+\.tsx?)\((\d+),\d+\): error (TS\d+)',result.stdout+result.stderr)
        detail='; '.join(f'{p}:{line} {code}' for p,line,code in matches[:2])
        if name=='tests':
            detail='assertions='+str(len(re.findall(r'^not ok ',result.stdout,re.M)))
