...
const f={rpcCalls:[],deletes:[],updates:[],invoiceCount:0,existing:null,snapshot:null,rows:[],deleteFails:false,updateCount:1,ackError:false,current:null};globalThis.__retentionTest=f;
const modules={
 'server-only':'export {};',
 'archiver':'export {};',
 ...
 '@/lib/supabase/admin':`export function getSupabaseAdmin(){const f=globalThis.__retentionTest;return {async rpc(name,args){f.rpcCalls.push(name);return {data:f.snapshot,error:null};},from(table){let updating=false;const q={select(){return q;},eq(){return q;},is(){return q;},order(){return q;},limit(){return q;},update(patch,options){if(options?.count!=='exact')throw Error('Exact count required');updating=true;f.updates.push(patch);return q;},async maybeSingle(){return {data:table==='storage_deletions'?f.current:f.existing,error:null};},then(resolve){return Promise.resolve(table==='invoices'?{count:f.invoiceCount,error:null}:updating?{count:f.updateCount,error:f.ackError?{}:null}:{data:f.rows,error:null}).then(resolve);}};return q;}};}`,
};
...
function setup(){f.rpcCalls=[];f.deletes=[];f.updates=[];f.invoiceCount=0;f.existing=null;f.rows=[];f.deleteFails=false;f.updateCount=1;f.ackError=false;f.current=null;f.snapshot={project:{id,archived_at:new Date().toISOString()},milestones:[],files:[]};}
...
test('zero-row acknowledgement already completed is skipped',async()=>{setup();f.rows=[{r2_key:'synthetic-key'}];f.updateCount=0;f.current={completed_at:'2026-09-12T00:00:00Z'};assert.deepEqual(await drainStorageDeletions(),{completed:0,failed:0});});
test('unknown missing or unsuccessful acknowledgement remains retryable',async()=>{for(const [count,current,error] of [[null,null,false],[0,null,false],[0,{completed_at:null},false],[1,null,true]]){setup();f.rows=[{r2_key:'synthetic-key'}];f.updateCount=count;f.current=current;f.ackError=error;assert.deepEqual(await drainStorageDeletions(),{completed:0,failed:1});}});
