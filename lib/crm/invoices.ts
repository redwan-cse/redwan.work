...
export async function rejectPayment(paymentId: string): Promise<CrmResult> { if (!validUuid(paymentId)) return crmError('Payment is no longer pending.'); const { error } = await getSupabaseAdmin().from('payments').update({ status: 'rejected' }).eq('id', paymentId).eq('status', 'submitted'); return error ? crmError('Invoice operation failed.') : { ok: true }; }
