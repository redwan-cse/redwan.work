...
async function threadMessages(ticketId: string) {
  const { data, error } = await getSupabaseAdmin().from('ticket_messages').select('id, body, created_at, profiles!ticket_messages_author_id_fkey ( full_name, role )').eq('ticket_id',ticketId).order('created_at',{ ascending:true });
  if (error) return null;
  return ((data ?? []) as unknown as Array<{ id: string; body: string; created_at: string; profiles: { full_name: string | null; role: string } | null }>).map(m => ({ id:m.id, body:m.body, created_at:m.created_at, author_name:m.profiles?.full_name ?? null, author_role:m.profiles?.role === 'admin' ? 'admin' as const : 'client' as const }));
}
export async function getTicketThread(ticketId: string) {
  ...
  if (error) return crmError('Could not load ticket.');
  if (!data) return crmError('Ticket not found.');
  const row = data as unknown as TicketJoin;
  const { data: user } = await admin.auth.admin.getUserById(row.client_id);
  const messages = await threadMessages(ticketId);
  if (!messages) return crmError('Could not load messages.');
  return { ok:true as const, ticket:mapTicket(row,user?.user?.email ?? ''), messages };
}
...
export async function getOwnTicketThread(clientId: string,ticketId: string) {
  ...
  const messages = await threadMessages(ticketId);
  if (!messages) return crmError('Could not load messages.');
  return { ok:true as const,ticket:data as PortalTicketRow & { client_id:string },messages };
}
