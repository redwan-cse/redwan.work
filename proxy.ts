...
function panelHome(role: string | undefined): string | null {
  if (role === 'admin') return '/admin'; if (role === 'client') return '/portal'; return null;
}
function readClaimRole(claims: unknown): string | undefined {
  ...
  const role = c?.app_metadata?.['role']; return typeof role === 'string' ? role : undefined;
}
function readClaimSub(claims: unknown): string | undefined {
  const sub = (claims as { sub?: unknown } | null | undefined)?.sub; return typeof sub === 'string' ? sub : undefined;
}
export async function proxy(request: NextRequest): Promise<NextResponse> {
  ...
  if (!url || !publishableKey) {
    ...
    loginUrl.searchParams.set('next', `${pathname}${search}`); return NextResponse.redirect(loginUrl);
  }
  ...
  try {
    ...
    const role = readClaimRole(data?.claims); const userId = readClaimSub(data?.claims);
    if (error || !userId) {
      if (isAuthPage) return response;
      loginUrl.searchParams.set('next', `${pathname}${search}`); return redirectWithCookies(loginUrl, response);
    }
    if (pathname === '/reset-password' || pathname.startsWith('/invite/')) return response;
    const { data: profile, error: profileError } = await supabase.from('profiles').select('role, is_active').eq('id', userId).maybeSingle();
    ...
    if (pathname.startsWith('/admin') && role !== 'admin') return redirectWithCookies(new URL(home, request.url), response);
    if (pathname.startsWith('/portal') && role !== 'client') return redirectWithCookies(new URL(home, request.url), response);
    return response;
  } catch { console.error('Route authority check unavailable.'); return unavailableResponse(response); }
}
...
function redirectWithCookies(target: URL, base: NextResponse): NextResponse { return carrySessionResponse(base, NextResponse.redirect(target)); }
