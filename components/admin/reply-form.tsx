...
export function ReplyForm({ticketId,latestHref}:{ticketId:string;latestHref?:string}) {
  ...
  return <form ref={formRef} className="space-y-3" onSubmit={e=>{e.preventDefault();if(pending||uploading)return;const data=new FormData(e.currentTarget);setError(null);startTransition(async()=>{try{const state=await replyToTicketAction(ticketId,{},data);if(state.error){setError(state.error);return;}formRef.current?.reset();if(latestHref)router.replace(latestHref+'#conversation');router.refresh();}catch{setError('Reply could not be sent. Please try again.');}});}}>
