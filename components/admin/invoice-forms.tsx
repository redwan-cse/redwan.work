...
import { addInvoiceItemAction, confirmPaymentAction, createDraftInvoiceAction, deleteInvoiceItemAction, rejectPaymentAction, sendInvoiceAction, updateDraftInvoiceAction, updateInvoiceItemAction, voidInvoiceAction } from '@/lib/crm/admin-actions';
...
export function NewInvoiceForm({ projects }: { projects: Array<{ id: string; name: string; client_name: string | null; client_email: string }> }) {
  ...
  function submit(formData: FormData, shouldSend: boolean) {
    setError(null);
    if (!items.some((item) => item.description.trim() && Number(item.qty) > 0 && cents(item.unit_price) >= 0)) {
      setError('Add at least one line item with a positive quantity and price.');
      return;
    }
    startTransition(async () => {
      const result = await createDraftInvoiceAction({ project_id: String(formData.get('project_id')), currency: String(formData.get('currency') || 'USD'), due_at: String(formData.get('due_at') || '') || null, payment_note: String(formData.get('payment_note') || '') || null });
      if (result.error || !result.invoiceId) { setError(result.error ?? 'Draft could not be created.'); return; }
      for (let position = 0; position < items.length; position += 1) {
        const item = items[position];
        if (!item.description.trim() && !item.unit_price) continue;
        const itemResult = await addInvoiceItemAction(result.invoiceId, {
          description: item.description,
          qty: Number(item.qty),
          unit_price_cents: cents(item.unit_price),
          position,
        });
        if (itemResult.error) { setError(itemResult.error); return; }
      }
      if (shouldSend) {
  ...
  return <form ref={formRef} action={(formData) => submit(formData, false)} className="space-y-5">
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-1.5"><Label htmlFor="invoice-currency">Currency</Label><Input id="invoice-currency" name="currency" defaultValue="USD" maxLength={3} required /></div>
    ...
    <fieldset className="space-y-3"><legend className="text-sm font-medium">Line items</legend>{items.map((item, index) => <div key={index} className="grid gap-2 sm:grid-cols-[1fr_100px_130px_auto]"><div className="space-y-1"><Label htmlFor={`new-description-${index}`}>Description</Label><Input id={`new-description-${index}`} value={item.description} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, description: event.target.value } : row))} required={index === 0} maxLength={500} /></div><div className="space-y-1"><Label htmlFor={`new-qty-${index}`}>Qty</Label><Input id={`new-qty-${index}`} value={item.qty} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, qty: event.target.value } : row))} type="number" step="0.001" min="0.001" required={index === 0} /></div><div className="space-y-1"><Label htmlFor={`new-price-${index}`}>Unit price</Label><Input id={`new-price-${index}`} value={item.unit_price} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, unit_price: event.target.value } : row))} type="number" step="0.01" min="0" required={index === 0} /></div><div className="flex items-end"><Button type="button" size="sm" variant="ghost" onClick={() => setItems((current) => current.filter((_, rowIndex) => rowIndex !== index))} disabled={items.length === 1}>Remove</Button></div></div>)}<Button type="button" size="sm" variant="outline" onClick={() => setItems((current) => [...current, { description: '', qty: '1', unit_price: '' }])}>Add line item</Button></fieldset>
...
export function DraftInvoiceForm({ invoice, items }: { invoice: { id: string; project_id: string; currency: string; due_at: string | null; payment_note: string | null }; items: InvoiceItemRow[] }) {
...
export function DeleteItemButton({ itemId }: { itemId: string }) { const [pending, startTransition] = useTransition(); const router = useRouter(); return <Button type="button" size="sm" variant="ghost" disabled={pending} onClick={() => startTransition(async () => { const result = await deleteInvoiceItemAction(itemId); if (!result.error) router.refresh(); })}>{pending ? 'Removing…' : 'Remove'}</Button>; }
