...
export function ReplyForm({ticketId}:{ticketId:string}) {
  ...
  return <form ref={formRef} className="space-y-3" onSubmit={e=>{e.preventDefault();if(pending||uploading)return;const data=new FormData(e.currentTarget);setError(null);startTransition(async()=>{const state=await clientReplyAction(ticketId,{},data);if(state.error){setError(state.error);return;}formRef.current?.reset();});}}>
